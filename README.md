# Demo of Gradual React Upgrades (with Next.js)
...
This demo is built with Next.js.
...
- How to install two versions of React in a single app with yarn side by side.
...
File structure is extremely important in this demo. It has a direct effect on which code is going to use which version of React. This particular demo is using Next.js with minor changes to its build config.
...
- **`package.json`**: The root `package.json` is a place for build dependencies (such as `next`) and any React-agnostic libraries (for example, `lodash`, `immer`, or `redux`). We **do** include React as `next` requires it.
- **`src/legacy/package.json`**: This is where we declare the `react` and `react-dom` dependencies for the "legacy" trees. In this demo, we're using React 16.8 (although, as noted above, we could downgrade it further below). This is **also** where we specify any third-party libraries that use React. For example, we include `react-redux` in this example.
- **`src/modern/package.json`**: This is where we declare dependencies for the "modern" trees. In this demo, we're using React 17. Here, we also specify third-party dependencies that use React and are used from the modern part of our app. This is why we have `react-redux` in this file. (Their versions don't strictly have to match their `legacy` counterparts, but features that rely on context may require workarounds if they differ.)

The `scripts` in the root `package.json` are set up so that when you run `yarn` in it, it also runs `yarn` in both `src/legacy` and `src/modern` folders.

**Note:** This demo is set up to use a few third-party dependencies (Redux). These are not essential, and you can remove them from the demo. They are included so we can show how to make them work with this approach.
...
To encourage only loading the older React when necessary, this demo includes a helper that works with `dynamic`. For example, `src/modern/AboutPage.js`, simplified, looks like this:
...
As a result, only if the `AboutPage` (and as a result, `<Greeting />`) gets rendered, we will load the bundle containing the legacy React and the legacy `Greeting` component.
...
This breaks third-party libraries like React Redux or Next Router, as well as any of your own usage of Context (for example, for theming).
